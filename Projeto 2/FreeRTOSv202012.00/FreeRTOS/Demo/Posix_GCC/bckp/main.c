...
int main( void )
{
   /* Do not include trace code when performing a code coverage analysis. */
    ...
    /*----------------------------------------------------*/
    //uiTraceStart();
    xTaskCreate(SendData,"Task Read Sensor",configMINIMAL_STACK_SIZE+1024,NULL,2,&xRead);
    xTaskCreate(PlantControlTask,"Task Control",configMINIMAL_STACK_SIZE*2,NULL,3,&xPlantControlTask);
    xTaskCreate(LEDTask,"CPU Usage",configMINIMAL_STACK_SIZE+1024,NULL,2,&xLEDTask);
...
void KeypadScanningTask(void *pvParameters){
    ...
    xLastWakeTime = xTaskGetTickCount();
    while(1){
        vTaskDelayUntil( &xLastWakeTime, DELAY_KEYBOARD );
...
void LEDTask(void *pvParameters){
    ...
    xLastWakeTime = xTaskGetTickCount();
    while(1){
        vTaskDelayUntil( &xLastWakeTime, DELAY_LED);
        if(xGetUsageCPU() > 85)
        {   
